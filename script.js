...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function updateProdiPaguInfo(userData) {
    ...
    try {
        ...
        activeAjuanQuery.forEach(doc => {
            totalDiajukan += Number(doc.data().Total) || 0;
        });

        const sisaPagu = paguAnggaran - totalDiajukan;
        const sisaPaguClass = sisaPagu < 0 ? 'text-danger fw-bold' : 'text-success';

        paguInfoArea.innerHTML = `
            <div class="d-flex flex-wrap justify-content-around text-center small">
                <div>
                    <span class="text-muted">Pagu Anggaran Anda:</span>
                    <strong class="d-block fs-6">Rp ${paguAnggaran.toLocaleString('id-ID')}</strong>
                </div>
                <div>
                    <span class="text-muted">Total Telah Diajukan:</span>
                    <strong class="d-block fs-6">Rp ${totalDiajukan.toLocaleString('id-ID')}</strong>
                </div>
                <div>
                    <span class="text-muted">Sisa Pagu:</span>
                    <strong class="d-block fs-6 ${sisaPaguClass}">Rp ${sisaPagu.toLocaleString('id-ID')}</strong>
                </div>
            </div>
        `;
  ...
  document.getElementById('btn-submit-all-staged').addEventListener('click', async () => {
    ...
    try {
        ...
        const paguAnggaran = prodiDoc.data().Pagu_Anggaran || 0;
        ...
        if (paguAnggaran > 0) {
            ...
            let currentTotalAjuan = 0;
            activeAjuanQuery.forEach(doc => {
                currentTotalAjuan += Number(doc.data().Total) || 0;
            });

            const totalStaging = STATE.stagingList.reduce((sum, item) => sum + item.Total, 0);
            
            const projectedTotal = currentTotalAjuan + totalStaging;

            if (projectedTotal > paguAnggaran) {
                throw new Error(`Gagal. Total ajuan yang diajukan (Rp ${projectedTotal.toLocaleString('id-ID')}) akan melebihi pagu anggaran Anda (Rp ${paguAnggaran.toLocaleString('id-ID')}).`);
            }
        }
  ...
  document.getElementById('btn-update-ajuan').addEventListener('click', async () => {
    ...
    try {
        ...
        if (STATE.role === 'prodi') {
            ...
            if (paguAnggaran > 0) {
                const originalTotal = Number(dataBefore.Total) || 0;
                const activeAjuanQuery = await db.collection('ajuan').where('ID_Prodi', '==', STATE.id).where('Status', 'in', ['Menunggu Review', 'Diterima', 'Revisi']).get();
                let currentTotalAjuan = 0;
                activeAjuanQuery.forEach(doc => { currentTotalAjuan += Number(doc.data().Total) || 0; });
                const projectedTotal = currentTotalAjuan - originalTotal + newTotal;
                if (projectedTotal > paguAnggaran) {
                     throw new Error(`Gagal. Total ajuan (Rp ${projectedTotal.toLocaleString('id-ID')}) akan melebihi pagu Anda (Rp ${paguAnggaran.toLocaleString('id-ID')}).`);
                }
            }
        }
  ...
  function renderDashboardSummary(data, containerPrefix = 'dashboard-', chartPrefix = 'chart') { let totalDiajukan = 0, totalDiterima = 0; let statusCounts = { 'Menunggu Review': 0, 'Diterima': 0, 'Ditolak': 0, 'Revisi': 0 }; const rpdPerBulan = Array(12).fill(0); const realisasiPerBulan = Array(12).fill(0); data.forEach(ajuan => { totalDiajukan += Number(ajuan.Total) || 0; if (ajuan.Status) { statusCounts[ajuan.Status] = (statusCounts[ajuan.Status] || 0) + 1; } if (ajuan.Status === 'Diterima') { totalDiterima += Number(ajuan.Total) || 0; RPD_MONTHS.forEach((month, index) => { rpdPerBulan[index] += Number(ajuan[`RPD_${month}`]) || 0; realisasiPerBulan[index] += Number(ajuan[`Realisasi_${month}`]) || 0; }); } }); const totalRPD = rpdPerBulan.reduce((a, b) => a + b, 0); const totalRealisasi = realisasiPerBulan.reduce((a, b) => a + b, 0); document.getElementById(`${containerPrefix}total-diajukan`).textContent = 'Rp ' + totalDiajukan.toLocaleString('id-ID'); document.getElementById(`${containerPrefix}total-diterima`).textContent = 'Rp ' + totalDiterima.toLocaleString('id-ID'); document.getElementById(`${containerPrefix}total-rpd`).textContent = 'Rp ' + totalRPD.toLocaleString('id-ID'); document.getElementById(`${containerPrefix}total-realisasi`).textContent = 'Rp ' + totalRealisasi.toLocaleString('id-ID'); document.getElementById(`${containerPrefix}count-menunggu`).textContent = statusCounts['Menunggu Review']; document.getElementById(`${containerPrefix}count-diterima`).textContent = statusCounts['Diterima']; document.getElementById(`${containerPrefix}count-ditolak`).textContent = statusCounts['Ditolak']; document.getElementById(`${containerPrefix}count-revisi`).textContent = statusCounts['Revisi']; const paguCard = document.getElementById('dashboard-total-pagu-card'); const paguValueEl = document.getElementById('dashboard-total-pagu'); const paguLabelEl = document.getElementById('dashboard-pagu-label'); paguCard.style.display = 'none'; if (STATE.role === 'direktorat') { const totalPagu = (STATE.allProdi || []).filter(p => p.Role === 'prodi').reduce((sum, p) => sum + (Number(p.Pagu_Anggaran) || 0), 0); paguValueEl.textContent = 'Rp ' + totalPagu.toLocaleString('id-ID'); paguLabelEl.textContent = 'TOTAL PAGU PRODI/UNIT'; paguCard.style.display = 'block'; } else if (STATE.role === 'prodi' && STATE.currentUserData) { const pagu = STATE.currentUserData.Pagu_Anggaran || 0; paguValueEl.textContent = 'Rp ' + pagu.toLocaleString('id-ID'); paguLabelEl.textContent = 'PAGU ANGGARAN SAYA'; paguCard.style.display = 'block'; } setupChart(`${chartPrefix}RPDvsRealisasi`, 'bar', { labels: RPD_MONTHS, datasets: [{ label: 'Realisasi (Rp)', data: realisasiPerBulan, backgroundColor: 'rgba(255, 193, 7, 0.7)' }, { label: 'RPD (Rp)', data: rpdPerBulan, backgroundColor: 'rgba(13, 110, 253, 0.6)' }] }, { responsive: true, scales: { x: { stacked: false }, y: { stacked: false, beginAtZero: true } } }); 
  ...
  function renderProdiDetailReport(prodiId, prodiData, container) { const prodiSafeId = prodiId.replace(/[^a-zA-Z0-9]/g, ''); const prodiHtml = `<div class="col-12 col-lg-6"><div class="card p-3 h-100"><h6 class="card-title mb-3 border-bottom pb-2"><strong><i class="bi bi-building"></i> Laporan: ${escapeHtml(prodiId)}</strong></h6><div class="row g-3"><div class="col-6"><div class="card p-2 h-100"><div class="small text-muted">PAGU ANGGARAN</div><strong id="prodi-${prodiSafeId}-pagu" class="fs-6 text-primary"></strong></div></div><div class="col-6"><div class="card p-2 h-100"><div class="small text-muted">SISA PAGU</div><strong id="prodi-${prodiSafeId}-sisa-pagu" class="fs-6"></strong></div></div><div class="col-6"><div class="card p-2 h-100"><div class="small text-muted">DIAJUKAN</div><strong id="prodi-${prodiSafeId}-diajukan" class="fs-6"></strong></div></div><div class="col-6"><div class="card p-2 h-100"><div class="small text-muted">DITERIMA</div><strong id="prodi-${prodiSafeId}-diterima" class="fs-6"></strong></div></div><div class="col-6"><div class="card p-2 h-100"><div class="small text-muted">RPD</div><strong id="prodi-${prodiSafeId}-rpd" class="fs-6"></strong></div></div><div class="col-6"><div class="card p-2 h-100"><div class="small text-muted">REALISASI</div><strong id="prodi-${prodiSafeId}-realisasi" class="fs-6"></strong></div></div><div class="col-12"><div class="card p-2 text-center bg-light"><div class="small text-muted">% REALISASI (DARI RPD)</div><strong id="prodi-${prodiSafeId}-persen" class="fs-4 text-primary">0.0%</strong><div class="progress mt-1" role="progressbar" style="height: 5px;"><div id="prodi-${prodiSafeId}-persen-bar" class="progress-bar" style="width: 0%;"></div></div></div></div></div><div class="mt-3"><h6 class="text-center small text-muted">Progress Bulanan</h6><canvas id="chart-prodi-${prodiSafeId}-progress"></canvas></div></div></div>`; container.insertAdjacentHTML('beforeend', prodiHtml); const prodiInfo = (STATE.allProdi || []).find(p => p.ID_Prodi === prodiId); const pagu = prodiInfo ? (Number(prodiInfo.Pagu_Anggaran) || 0) : 0; let totalDiajukan = 0, totalDiterima = 0, totalRPD = 0, totalRealisasi = 0; const rpdPerBulan = Array(12).fill(0); const realisasiPerBulan = Array(12).fill(0); prodiData.forEach(ajuan => { totalDiajukan += Number(ajuan.Total) || 0; if (ajuan.Status === 'Diterima') { totalDiterima += Number(ajuan.Total) || 0; RPD_MONTHS.forEach((month, index) => { const rpdVal = Number(ajuan[`RPD_${month}`]) || 0; const realVal = Number(ajuan[`Realisasi_${month}`]) || 0; totalRPD += rpdVal; totalRealisasi += realVal; rpdPerBulan[index] += rpdVal; realisasiPerBulan[index] += realVal; }); } }); const sisaPagu = pagu - totalDiterima; const sisaPaguClass = sisaPagu < 0 ? 'text-danger' : 'text-success'; document.getElementById(`prodi-${prodiSafeId}-pagu`).textContent = 'Rp ' + pagu.toLocaleString('id-ID'); const sisaPaguEl = document.getElementById(`prodi-${prodiSafeId}-sisa-pagu`); sisaPaguEl.textContent = 'Rp ' + sisaPagu.toLocaleString('id-ID'); sisaPaguEl.className = `fs-6 fw-bold ${sisaPaguClass}`; const persentaseRealisasi = totalRPD > 0 ? (totalRealisasi / totalRPD) * 100 : 0; document.getElementById(`prodi-${prodiSafeId}-diajukan`).textContent = 'Rp ' + totalDiajukan.toLocaleString('id-ID'); document.getElementById(`prodi-${prodiSafeId}-diterima`).textContent = 'Rp ' + totalDiterima.toLocaleString('id-ID'); document.getElementById(`prodi-${prodiSafeId}-rpd`).textContent = 'Rp ' + totalRPD.toLocaleString('id-ID'); document.getElementById(`prodi-${prodiSafeId}-realisasi`).textContent = 'Rp ' + totalRealisasi.toLocaleString('id-ID'); const persenEl = document.getElementById(`prodi-${prodiSafeId}-persen`); if (persenEl) persenEl.textContent = persentaseRealisasi.toFixed(1) + '%'; const persenBarEl = document.getElementById(`prodi-${prodiSafeId}-persen-bar`); if (persenBarEl) persenBarEl.style.width = `${Math.min(persentaseRealisasi, 100)}%`; setupChart(`chart-prodi-${prodiSafeId}-progress`, 'bar', { labels: RPD_MONTHS, datasets: [{ label: 'Realisasi (Rp)', data: realisasiPerBulan, backgroundColor: 'rgba(255, 193, 7, 0.7)' }, { label: 'RPD (Rp)', data: rpdPerBulan, backgroundColor: 'rgba(13, 110, 253, 0.6)' }] }, { responsive: true, scales: { y: { beginAtZero: true } }, plugins: { legend: { display: false } } }); }
